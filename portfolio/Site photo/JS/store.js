...
function setCookie(cname, cvalue, exdays) {
  ...
  cartArticles.forEach(function (v) {
    items += '<p id="' + v.id + '"><a href="' + v.url + '">' + v.name + '<br><small>Quantité : <span class="qt">' + v.qt + '</span></small></a></p>';
  });
  ...
  $('.add-to-cart').click(function () {
    ...
    if (newArticle) {
      $('#cart-dropdown').prepend('<p id="' + id + '"><a href="' + url + '">' + name + '<br><small>Quantité : <span class="qt">' + qt + '</span></small></a></p>');
  ...
  if (window.location.pathname == '/franckydaily/panier.html' || '../HTML/panier.html') {
    var items = '';
    var subTotal = 0;
    var total;
    ...
    cartArticles.forEach(function (v) {
      ...
      items += '<tr data-id="' + v.id + '">\
                 ...
                 <td>' + v.price + '€</td>\
                 <td><button class="qt-minus rounded-circle btn btn-info"><i class="fas fa-minus"></i></button> <span class="qt"> ' + v.qt + '</span> <button class="qt-plus rounded-circle btn btn-info"><i class="fas fa-plus"></i></button> \
                 <button class="delete-item btn btn-danger"><i class="fas fa-trash"></i></button></td></tr>';
    ...
    $('.qt-minus').click(function () {
      var $this = $(this);
      var qt = parseInt($this.next('.qt').html());
      var id = $this.parent().parent().attr('data-id');
  
      if (qt > 1) {
        // maj qt
        inCartItemsNum -= 1;
        $this.next('.qt').html(qt - 1);
        $('#in-cart-items-num').html(inCartItemsNum);
        $('#' + id + ' .qt').html(qt - 1);
